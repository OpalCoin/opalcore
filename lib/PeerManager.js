...
var opalcoreDefaults = require('../config');
...
function PeerManager(config) {
  ...
  this.config = config || {};
  for (var i in opalcoreDefaults)
    if (opalcoreDefaults.hasOwnProperty(i) && this.config[i] === undefined)
      this.config[i] = opalcoreDefaults[i];
